...
export default function NavBar(){
  ...
  const possibleColors = ["blue", "orange", "violet", "green"];
  const changeThemeColors = e =>{
    let newColorIndex =  possibleColors.findIndex(el => el === color.current) +1
    possibleColors[newColorIndex] === undefined &&  (newColorIndex = 0) 
    color.current = possibleColors[newColorIndex] 
    document.body.id = color.current   
  }
      ...
      <li className="nav-dropdown"><a href="#" style={{cursor: 'default'}}>Theme</a>
        <ul className="dropdown">
          <li className="dropdown-option">
            ...
            <button id="feeling-lucky" className="link" onClick = {changeThemeColors}>Change Color</button>
          </li>
          ...
          <li className="dropdown-option">
            ...
            <button style={{color: 'var(--black)', cursor: 'default'}}> Night Mode</button>
